...
class ControlPanel:
    def __init__(self, root):
        ...
        self.root.after(500, self._check_first_launch)

        if self.settings.get('start_minimized'):
            self.root.withdraw()
        if self.settings.get('auto_start_engine'):
            self.root.after(1500, lambda: self._toggle(manual=False))
        if self.settings.get('force_startle_on_launch'):
            self.root.after(2500, lambda: self.engine.trigger_event("startle"))
...
Enjoy the pink fog... 💕"""
    ...
    def _build_main(self, p):
        ...
        self.sl_vfreq, self.lb_vfreq = self._slider(cvid, "Per Hour", 1, 60, tip="Videos per hour.")
